...
<center><h2> <font color="black"> ENEM </h2></center></font>
...
<?php
 ...
 $query = $connect-> prepare ('SELECT id_grupo, descricao_grupo FROM grupo');
 $query->execute();
 $grupos = $query->fetchAll();
 foreach ($grupos as $grupo) {
    
    echo  '<a class="list-group-item list-group-item-action "
    id="list-home-list" data-toggle="list" href="?id_grupo='.$grupo['id_grupo'].'">'.$grupo['descricao_grupo'].'</a>';
  ...
  $query = $connect->prepare ('SELECT descricao_grupo FROM grupo WHERE id_grupo='.$id);
  $query->execute();
  $grupo = $query->fetch();
...
echo '<br><br>';
  ...
  try {
    $query2 = $connect->prepare ('SELECT texto, id_questao, texto_adicional, resp_1, resp_2, resp_3,resp_4, resp_5 FROM questao WHERE id_grupo='.$id);
    $query2->execute();
  ...
  echo '<form action="" method="POST">';
    ...
    foreach ($data as $value) {
      echo "<INPUT TYPE='hidden' NAME='id_".$value['id_questao']."' VALUE='".$value['id_questao']."' <br><br>'";
      echo $cont++ .' - '. $value ['texto']. '<br><br> ';
      echo $value ['texto_adicional']. '<br><br>';
      ...
      foreach ($resps as $resp) {
        echo $item.") <INPUT TYPE='radio' NAME='".$value['id_questao']."' VALUE='".$resp[0]."|".$resp[1]."'> ".$resp[0]."<br>"; 
        $i++;
        $item++;        
      }
    }  
